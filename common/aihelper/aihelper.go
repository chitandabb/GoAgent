...
func NewAIHelper(model_ AIModel, SessionID string) *AIHelper {
	return &AIHelper{
		...
		// The default MESGuard path persists the message before returning.
		// RabbitMQ remains available as an optional legacy integration, but it is
		// no longer required for chat correctness or process startup.
