...
- 当前仓库只实现 `/healthz` Web 基础骨架；本文中的认证、工单、附件、诊断、报告和管理接口尚未实现。
...
本文审阅通过后停止继续铺设主设计文档，进入 M0 代码：数据库迁移命令、用户与 Session、Argon2id、CSRF、Repository/事务约定和认证接口。具体 Handler 开始实现时同步建立 `api/openapi.yaml`。
