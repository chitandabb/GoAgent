...
| React + Nginx | 未实现 | M1 |
| Eino Agent 与 StepFun | 当前单 ADK Agent、启动 Epoch 固定 ToolProfile、执行期 RunAccess/ResourceGrant Guard、冻结 InvestigationPolicy、Skill 渐进加载、usage 和 Evidence Gate 已实现；旧 TaskScope/Capability/request_scope 体系已硬切删除，旧 paired 指标待生产入口 v2 重测；P7 已接入正式 Worker 执行、报告生成/读取、TaskEvent 补读、SSE、取消、恢复和报告反馈基础链路 | M1 |
