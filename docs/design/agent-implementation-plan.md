...
正式 Catalog 扫描/审核/发布管理仍未开始；DiagnosisTask/Worker 已把运行时证据接入正式持久化。
...
- 会话不固定主工单或诊断任务；右侧卷宗选择只作为本轮消息的结构化引用，绑定工单不自动运行；
- 用户明确要求诊断时，独立 Conversation Agent 通过受控命令创建与会话解耦的异步任务；原会话可继续承载知识问题、任务状态和后续补充；
