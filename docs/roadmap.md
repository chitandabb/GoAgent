...
## Current Stage: Backend Evaluation Closed; Embedding Quota Governance Next
...
- [x] OpenCode Go 主模型硬切生产默认（2026-08-16）：
      ...
      保持一致，配置合同测试显式以 stepfun-main 为对照。正式 45 Case Tool
      Selection 与 20 Case Conversation Text-to-SQL 已在 clean revision `7c4f5d6`
      完成，详细事实分别见 [`evaluations/tool-selection-v4.md`](evaluations/tool-selection-v4.md)
      与 [`evaluations/text-to-sql-conversation-v2.md`](evaluations/text-to-sql-conversation-v2.md)。
...
- [x] 完成 OpenCode Go 生产入口 45 Case Tool Selection 与 20 Case Conversation
      Text-to-SQL 正式全量复测；两个 SQL 失败已转入下一治理切片。指标、异常、
      运行身份与可复现命令只维护在上述两份评测事实文档。
...
- [x] Stabilize Conversation user-message prompt bytes across turns: every user message now reuses one
      ...
      label for that change was `conversation-v8`; historical `conversation-v7` evaluation records remain unchanged.
- [x] Govern the two failures from the 20-Case Text-to-SQL run: `conversation-v9` states that
      row/record counts use `COUNT(*)` and that `COUNT(column)` excludes NULL values; Conversation
      executes `execute_readonly_query` at most once per turn. A second request consumes Tool-call
      budget, records a failed execution, skips the executor and returns a structured recoverable
      `blocked` result to the model when budget remains; exhausted budget takes fail-closed precedence.
      Evaluation records the known policy block without SQL text/hash and still marks a two-query
      model sequence invalid. Local red/green tests pass. On clean revision `2b7a257`, one formal
      single-Case run for each prior failure completed with exactly one readonly query and passed
      Tool sequence, execution, answer and end-to-end checks; `sql-customer-distribution` generated
      `COUNT(*)`. A later full 20-Case formal run on clean revision `fb92383` reached only 13/20 (65%):
      seven `invalid_tool_sequence` failures showed that targeted 2/2 success did not generalize, while
      authorization, QueryGuard and SQL Server remained fail closed. The resume therefore does not use
      v8's historical 90% as a current claim or promote the targeted runs to 20/20; details remain in
...
- [x] Complete the clean Tool Selection production-entry re-test on the v4/evaluation-wide-v2 contract. The representative three-Case run at revision `ab1026c` remains the protocol checkpoint; the completed 45-Case formal result at revision `7c4f5d6` is the current quality fact and is recorded once in [`evaluations/tool-selection-v4.md`](evaluations/tool-selection-v4.md). Natural-language Conversation Text-to-SQL remains tracked separately in [`evaluations/text-to-sql-conversation-v2.md`](evaluations/text-to-sql-conversation-v2.md).
- [x] Govern production Embedding RPM/TPM and retry behavior. Each process shares one governed client across
      retrieval/cache/ingestion consumers; checked-in four-process defaults aggregate to 800 RPM / 600000
      estimated TPM, retries are structured and bounded, and cost-sensitive evaluation tools force one attempt.
      A single-call production smoke on revision `fb92383` returned one 1024-dimensional `text-embedding-v4`
      vector with 6 reported Tokens; the invalid 40-document Provider pair was not rerun.
