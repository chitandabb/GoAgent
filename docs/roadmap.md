...
- [ ] Complete v2 production-entry fixed sets. A clean post-commit pilot on revision `3d40582` completed one ticket case in both arms with correct Skill/first Tool, 100% completion and zero forbidden calls (4 model calls, 18,384 total Token), but invalidated the current Token paired variable: the deployment's `evaluation-wide-v1` arm exposed 7 business Tools while `diagnosis-default` exposed 9 Tools after real Skill Middleware, so experiment prompt Token increased from 5,991 to 9,536 (+59.17%) and duration increased from 12.08s to 14.22s. Natural-language Conversation Text-to-SQL 的 StepFun 与 OpenCode Go 历史三 Case 基线均为 2/3（66.7%），但失败 Case 不同；OpenCode Go 使用 12 次模型调用和 27,424 Token，较 StepFun 描述性基线低约 33.3%，平均耗时则高约 30.2%。Observation v3 已补齐 schema search 诊断，且原失败 `sql-urgent-count` 的一次受控复测端到端通过，但历史失败未复现，不构成生产策略已修复的证据。当前保持 Prompt、Tool Schema 和调用上限不变；Tool Selection baseline 装配可比性和完整 Text-to-SQL 固定集仍未完成。详情：[`evaluations/text-to-sql-conversation-v2.md`](evaluations/text-to-sql-conversation-v2.md)。
