...
- [x] Complete the clean Tool Selection production-entry re-test on the v4/evaluation-wide-v2 contract. On revision `75ba56c`, OpenCode Go `deepseek-v4-flash` with explicit `toolChoiceMode=absent` completed `ticket-read-01`: both wide and production selected `read_external_case`, both were correct, `PairedCases=1`, `UnpairedRuns=0`, and all invalid/out-of-whitelist/failure rates were zero. Three Generate calls reported 8,167 Tokens in total. Production reduced paired Prompt Tokens by 4.18%, Tool Schema Tokens by 4.44%, and Schema bytes by 4.64% versus wide. This is a single-Case protocol/entry acceptance result, not a 45-Case quality claim. Natural-language Conversation Text-to-SQL remains tracked separately in [`evaluations/text-to-sql-conversation-v2.md`](evaluations/text-to-sql-conversation-v2.md).
