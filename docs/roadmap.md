...
- [ ] Run v2 production-entry fixed sets (Conversation SQL/`turn_context` wiring has landed; the formal run still requires a clean Git revision and post-commit execution). Record the generic v2 identity contract (`observationSchemaVersion`/`toolProfileId`/`toolSchemaFingerprint`/`modelProfileFingerprint`/`implementationRevision`/`implementationDirty`); pair baseline (`evaluation-wide-v1`) with experiment (`diagnosis-default`) only under identical clean identity, never requiring the arm-specific Tool Profile contracts to be equal. Re-measure resume item 1 and trigger Text-to-SQL from natural-language Conversation input for resume item 2.
