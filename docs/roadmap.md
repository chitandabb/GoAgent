...
- [x] Tool Selection v3: the evaluation experiment side resolves the fixed `diagnosis-default` Profile instead of `ToolsFor(TaskScope)`; observations carry an explicit `observationSchemaVersion` (`tool-selection-observation-v3`) with variants `wide`/`production` and record `toolProfileId` (production=`diagnosis-default`, wide=`evaluation-wide-v2`), `modelVisibleNames`, actual Schema names/hash, model Profile fingerprint and implementation revision. Wide/production comparability is fail-closed before any Provider is created: `VerifyToolSelectionComparability` verifies shared Tool Schema equality, the wide arm being a strict superset, and a single stable `comparisonFingerprint` with non-empty non-overlapping `sharedToolNames`/`baselineOnlyToolNames`. Implementation identity is fail-closed (dirty/unknown refused in formal mode, `-allow-dirty` only for local smoke), and wide/production arms pair only when schema version, model Profile fingerprint and revision/dirty state all match and neither arm is dirty. Historical v1/v2 Tool Selection data remains `retest_needed`.
...
- [ ] Complete production-entry re-test on the v3/evaluation-wide-v2 contract. Schema comparability assembly is DONE: the wide arm is the deployment-wide `evaluation-wide-v2` union ({production business Tools} ∪ {baseline-only Tools}) fed through the same Eino Skill Middleware as the experiment arm, `VerifyToolSelectionComparability` fails closed before any Provider creation, and the Generic paired baseline arm is byte-identical to the experiment arm in the final system message (same production SystemInstruction + same ticket-diagnosis entry Skill full text + the same non-empty `task_context` generated once per case by `BuildDiagnosisRunContext` with the real production `diagnosis-default` ceiling, same model Profile and budget; only the Tool Profile/Schema differ). A cost gate is DONE and is fail-closed before any Provider/remote-connection factory: `-allow-provider-calls`/`-max-cases`/`-max-provider-calls`/`-max-provider-tokens` (+ exact `-case-id` selection on the full validated dataset; default 45 cases × 3 calls/case = 135 call upper bound for Tool Selection) with a pre-run printed budget. The Tool Selection Token hard upper bound is derived from the evaluated named Profile's `contextWindowTokens` (per call ≤ window, per case = 3 × window, total = cases × 3 × window, multiplication overflow fails closed) — the fixed 16K input assumption is retired. The Evidence Gate fixed 30 reviewed Case target caps only the evidence-gate comparison; tool-selection case count is bounded only by dataset size, explicit `-max-cases` and the call/Token authorizations. A clean `f5831a4` single-Case probe (`ticket-read-01`) established that the runtime/preflight Schema hash failure was a **Schema mutability defect, not a model-quality failure**: the Eino OpenAI ACL's `toTools` sorts `jsonSchema.Required` in place while `schema.ParamsOneOf.ToJSONSchema` returns the internal pointer for the JSONSchema representation, so binding the wide arm mutated shared ToolInfo held by `InferTool.Info()` and drifted the production arm's hash before any Provider request. In that probe the wide arm performed one formal Provider call and selected `read_external_case` correctly; the production arm failed closed before its Provider request, and no reducible Observation/Summary was produced. The fix lives on the **model Adapter seam**: `internal/platform/chatmodel` now returns every model wrapped in a schema-isolating decorator that deep-clones `[]*schema.ToolInfo` (via `ToolInfo`'s own MarshalJSON/UnmarshalJSON, preserving both params/jsonschema representations) before `WithTools` and before call-time `model.WithTools(...)` options in `Generate`/`Stream`, and re-wraps bound models; Catalog/Skill/Runner interfaces are unchanged and the runtime/preflight hash checks are untouched. The cloud re-test of the same Case on the v3 contract remains **pending** and must not be claimed as passed until it runs on a clean revision. What remains: the formal small-sample Provider re-test on the v3 contract (the old `3d40582` pilot used the retired `evaluation-wide-v1` contract and is not comparable). Natural-language Conversation Text-to-SQL 的 StepFun 与 OpenCode Go 历史三 Case 基线均为 2/3（66.7%），但失败 Case 不同；OpenCode Go 使用 12 次模型调用和 27,424 Token，较 StepFun 描述性基线低约 33.3%，平均耗时则高约 30.2%。Observation v3 已补齐 schema search 诊断，且原失败 `sql-urgent-count` 的一次受控复测端到端通过，但历史失败未复现，不构成生产策略已修复的证据。当前保持 Prompt、Tool Schema 和调用上限不变；完整 Text-to-SQL 固定集仍未完成。详情：[`evaluations/text-to-sql-conversation-v2.md`](evaluations/text-to-sql-conversation-v2.md)。
