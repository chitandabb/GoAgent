...
const (
	MessageType   = "conversation.turn.execute"
	SchemaVersion = 1
)

var ErrInvalidMessage = errors.New("conversation worker message is invalid")
...
func ParseMessage(input IncomingMessage) (Message, error) {
	...
	turnID, err := uuid.Parse(payload.TurnID)
	if err != nil {
		return Message{}, fmt.Errorf("%w: turn id", ErrInvalidMessage)
	}
	...
	return Message{
		...
		TurnID: turnID, OccurredAt: occurredAt.UTC(),
	}, nil
}
