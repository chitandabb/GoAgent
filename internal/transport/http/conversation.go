...
func NewConversationRoutes(useCase conversationUseCase, auth, csrf gin.HandlerFunc) (*ConversationRoutes, error) {
	if useCase == nil || auth == nil || csrf == nil {
		return nil, errors.New("conversation route dependencies are nil")
	}
	return &ConversationRoutes{useCase: useCase, auth: auth, csrf: csrf}, nil
}
...
func conversationMessageResponseFrom(item conversation.Message) conversationMessageResponse {
	...
	return conversationMessageResponse{
		...
		CaseReferences: cases, TaskReferences: tasks, CreatedAt: item.CreatedAt.UTC(),
	}
}
