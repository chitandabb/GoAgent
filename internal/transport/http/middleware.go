...
func ErrorHandler() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if httpStatus(appErr.Code) >= 500 {
			log.Printf(
				"request failed, request_id=%s, code=%d, error=%v",
				RequestIDFromContext(c), appErr.Code, err,
			)
		}
		WriteError(c, appErr)
	}
...
func Recovery() gin.HandlerFunc {
	return gin.CustomRecovery(func(c *gin.Context, recovered any) {
		log.Printf("panic recovered, request_id=%s, error=%v", RequestIDFromContext(c), recovered)
		if !c.Writer.Written() {
