...
func TestReadAttachmentToolInjectsDiagnosisTaskContext(t *testing.T) {
	...
	// 诊断执行上下文：任务附件上下文 + 权威 v2 RunAccess（携带本任务的
	// 附件 Grant）。旧 TaskScope 双写已硬切删除。
	...
	if reader.userID == uuid.Nil || reader.messageID != taskID || reader.attachmentID != attachmentID {
