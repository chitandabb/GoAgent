...
func TestBuildToolSelectionLedgerPreservesDomainMetricsAndMissingUsage(t *testing.T) {
	...
	observations := []ToolSelectionObservation{
		...
		toolSelectionLedgerObservation("run-filtered", ToolSelectionFiltered, "missing_provider_usage", ModelUsage{}),
	}
	...
	if report.Records[1].Outcome != evaluationledger.OutcomeFailed || report.Records[1].Usage != nil {
		t.Fatalf("filtered ledger record = %+v", report.Records[1])
	}
	...
	if summary.Wide.Accuracy != 1 || summary.Filtered.FailedRuns != 1 || summary.PairedCases != 0 {
